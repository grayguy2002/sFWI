...
class Evaluator:
    ...
    def __call__(self, gt, measurement, x, reduction='mean'):
        """
            ...
            Parameters:
                gt (torch.Tensor or None): Ground truth tensor. If None, only unsupervised metrics are computed.
        ...
        for eval_fn_name, eval_fn in self.eval_fn.items():
            # 判断是否需要 gt
            if gt is None and eval_fn.requires_gt:
                continue  # 跳过需要 ground truth 的评估函数
