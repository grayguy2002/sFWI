...
class DiffusionData(ABC, Dataset):
    ...
    def get_data(self, size=16, sigma=2e-3, seed=None):
        # 如果提供了seed，设置随机种子
        if seed is not None:
            torch.manual_seed(seed)

        total_len = len(self)
        random_indices = torch.randint(0, total_len, (size,))
        data = torch.stack([self.__getitem__(i) for i in random_indices], dim=0)
